...
usage() {
    echo "Usage:"
    echo "  ./bash.sh                Run all test cases"
    echo "  ./bash.sh --single       Convert src/input.txt to src/output.xml"
    echo "  ./bash.sh --files <file1> [file2 ...]"
}
...
if [[ "$MODE" == "--files" ]]; then
    ...
    for input_path in "$@"; do
        if [[ "$input_path" = /* ]]; then
            ...
            INPUT_FILES+=("$ROOT_DIR/$input_path")
        fi
    done
fi
...
if [[ "$MODE" == "--files" ]]; then
    ...
    for input_file in "${INPUT_FILES[@]}"; do
        ...
        if [[ ! -f "$input_file" ]]; then
            echo "[FAIL] missing input: $input_file" >&2
            continue
        fi
        ...
        rm -f "$output_file"
        if java -cp "$CP" Main "$input_file" "$output_file" &&
    ...
    echo "Files converted: $success/$total"
